...
function save_search($filename, $searches) {
    ...
    if (empty($isapc)) {
        ...
        apc_store($_SESSION['user_id'].'_searches', $searches_content);
    }
}
...
if (!empty($_GET['searchmode'])) {
    ...
    if (empty($isapc)) {
        ...
        if ($searches_content = apc_fetch($_SESSION['user_id'].'_searches'))
                $searches = unserialize(gzuncompress($searches_content));
    }
    if (count($searches) > 0) {
    ...
    if ($rows > 0) {
        ...
        <table cellspacing="1" id="customization" style="border-spacing:2px 0px;cursor:pointer">
            <tr>
                ...
                <td class="alternating_row"><span class="ui-icon ui-icon-radio-<?php print $display == 'icons' ? 'off' : 'on'; ?>" style="float:left"></span>Icons&nbsp;</td>
