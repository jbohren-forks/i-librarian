...
var dooverlay=function(){
    ...
    $('#overlay').html('<img src="img/ajaxloader3.gif" alt="" style="margin-left:48%;margin-top:'+(-32+0.5*$(document).height())+'px">');
            ...
            "Clear": function() {
                $.get('search.php?newsearch=1',function(){
                    ...
                    $('#expertsearchform').find('input:radio').each(function(){
                        $(this).next('span').removeClass('ui-icon-radio-off').addClass('ui-icon-radio-on');
                    });
                });
            },
            "Close": function() {
    ...
    init:function(){
        ...
        $(".select_span").unbind().click(function (e) {
            ...
            if($input.is(':radio')) {
                ...
                $(this).closest('table').find('input[name="'+rname+'"]').next().removeClass('ui-icon-radio-off').addClass('ui-icon-radio-on');
                $span.removeClass('ui-icon-radio-on').addClass('ui-icon-radio-off');
        ...
        $('#addarticle-left').click(function(e) {
            ...
            if($t.hasClass('leftbutton') || $t.hasClass('select-import')) {
                if($t.attr('id')=='import-any') {
                    $.getScript('wpad.php',function(){
                        var proxystr=FindProxyForURL('', 'www.crossref.org');
                        $('#addarticle-right').load('yuiuploader.php?proxystr='+encodeURIComponent(proxystr),function(){
                            yuiuploader.init()
                            });
                    });
                } else {
...
var yuiuploader={
    init:function(){
        ...
        var fd,fileList,prog,tot=0,compl=0;
        ...
        YAHOO.util.Event.onDOMReady(function () {
            var uiLayer = YAHOO.util.Dom.getRegion('select-button');
            var overlay = YAHOO.util.Dom.get('uploaderOverlay');
            YAHOO.util.Dom.setStyle(overlay, 'width', 1+uiLayer.right-uiLayer.left + "px");
            YAHOO.util.Dom.setStyle(overlay, 'height', 1+uiLayer.bottom-uiLayer.top + "px");
        });
        YAHOO.widget.Uploader.SWFURL = "js/yui/assets/uploader.swf";
        var uploader = new YAHOO.widget.Uploader("uploaderOverlay");

        uploader.addListener('contentReady', handleContentReady);
        uploader.addListener('click',onClick);
        ...
        function upload() {
            if (fileList != null && tot>0) {
                dooverlay();
                var ref=location.href.replace('index2.php','yuiuploader.php');
                uploader.setSimUploadLimit(1);
                uploader.uploadAll(ref+'?'+fd);
            } else {
                $.jGrowl('Select files to import first.');
            }
        }
        function onClick() {
            ...
            tot=0;
            compl=0;
            $('#file-count').text('0 files');
            $("#import-button").button('disable');
        }
        function onFileSelect(event) {
            if('fileList' in event && event.fileList != null) {
                fileList = event.fileList;
                $.each(fileList, function(ind,val){
                    tot=tot+1
                });
                $('#file-count').text(tot+' files');
                $("#import-button").button('enable');
            }
        }
        function onUploadResponse(event) {
            $.jGrowl(event.data);
            compl=compl+1,prog=Math.round(100*compl/tot);
            if(prog=='100') {
                $.jGrowl('Batch upload has finished.');
                clearoverlay();
                onClick();
            }
        }
...
var synchronize={
    init:function(){
        $("input:submit").button();
        jQuery("<img>").attr("src", "img/ajaxloader2.gif");
        $('form').ajaxForm({
            beforeSubmit: function(arr) {
                if(arr[0]['value']=='') return false;
                dooverlay();
...
var rename_journal={
    init:function(){
        ...
        $('#edit-journal-list').unbind().click(function(e){
            ...
            $t.next('div.journal-name-child').css('padding-left','12px').html('<img src="img/ajaxloader2.gif">')
        ...
        $(".rename-journal-button").unbind().click(function(){
            if($(this).next('span').is(':hidden')) {
                ...
                $(this).parent('div').css('padding-left','12px').html('<img src="img/ajaxloader2.gif">');
...
var categories={
    init:function(){
        $("td.select_span").click(function () {
            ...
            if($input.is(':radio')) {
                $input.prop('checked',true);
                $(this).closest('table').find('span.ui-icon-radio-off').removeClass('ui-icon-radio-off').addClass('ui-icon-radio-on');
                $span.removeClass('ui-icon-radio-on').addClass('ui-icon-radio-off');
            } else if($input.is(':checkbox')) {
                if($span.hasClass('ui-icon-close')) {
                    $input.prop('checked',true);
...
var desktop={
    init:function(openproj){
        ...
        $('table.projectheader').click(function(){
        ...
        $("table.removeuser").click(function(){
    ...
    init:function(sel,divhref){
        ...
        $('#display-content').click(function(event) {
            ...
            if($(t).hasClass('expander')) {
                ...
                if($(t).hasClass('ui-icon-circlesmall-plus')) {
                    $(t).removeClass('ui-icon-circlesmall-plus').addClass('ui-icon-circlesmall-minus');
                } else {
                    $(t).removeClass('ui-icon-circlesmall-minus').addClass('ui-icon-circlesmall-plus');
                }
            } else
            if($(t).hasClass('ui-icon-star')) {
                ...
                $.ajax({
                    ...
                    success: function(){
                        ...
                        if(item[2]==2) {
                            $parent.prev().andSelf().attr('class','star ui-state-error-text');
                            $parent.next().attr('class','star ui-priority-secondary');
                        }
                        if(item[2]==3) $parent.siblings('div.star').andSelf().attr('class','star ui-state-error-text');
                    }
                });
            } else
            if($(t).hasClass('update_clipboard')) {
                ...
                function(answer){
                    if (answer == "added") {
                        ...
                        $t.children('span').removeClass('ui-icon-circlesmall-close').addClass('ui-icon-check');
                    } else if (answer == "removed") {
                        $t.removeClass('remove ui-state-error-text').addClass('add');
                        $t.children('span').removeClass('ui-icon-check').addClass('ui-icon-circlesmall-close');
                    } else if (answer.substr(0,5) == "Error") {
                        $.jGrowl(answer, {
                            theme:'jgrowl-error'
                        });
        ...
        $('#customization').click(function(e){
            ...
            e.stopPropagation();
            if($target.hasClass('ui-icon-radio-off')) return false;
            if($target.find('span').hasClass('ui-icon-radio-off')) return false;
        ...
        $('#displaybutton').click(function(){
            ...
            $('#customization').find('span.ui-icon-radio-off').closest('td').show();
        });
            ...
            buttons: {
                ...
                "Reset": function() {
                    ...
                    $('#exportform input:radio').each(function(){
                        $(this).next('span').removeClass('ui-icon-radio-off').addClass('ui-icon-radio-on');
                        if($(this).is(':checked')) $(this).next('span').removeClass('ui-icon-radio-on').addClass('ui-icon-radio-off');
                    });
                },
                "Close": function() {
                    $(this).dialog("close");
            ...
            if($(t).hasClass('ui-icon-star')) {
                ...
                $.ajax({
                    ...
                    success: function(){
                        ...
                        if(item[3]==3) $parent.siblings('div.star').andSelf().attr('class','star ui-state-error-text');
                    }
                });
            } else
            if($(t).hasClass('update_clipboard')) {
    ...
    init:function() {
        ...
        $('.video').click(function(){
            ...
            if($videocontainer.is(':hidden')) {
                $videocontainer.html('<div class="media-player">'
                    +'<video controls="controls" preload="none" autoplay="autoplay"> <source src="library/supplement/'+escape(file)+'" /> </video>'
                    +'<div class="media-state"></div>'
                    +'<div class="media-controls ui-corner-bottom">'
                    ...
                    +'</div>')
                .show();
                $videocontainer.find('div.media-player').jmeControl();
            } else {
        ...
        $('.audio').click(function(){
            ...
            if($audiocontainer.is(':hidden')) {
                $audiocontainer.html('<div class="media-player">'
                    +'<div class="alternating_row"'
                    +'style="padding:8px;border-top:1px solid #AFAEA9;border-left:1px solid #AFAEA9;border-right:1px solid #AFAEA9">'
                    +escape(file.substr(5))+'</div>'
                    +'<audio controls="controls" preload="none" autoplay="autoplay"> <source src="library/supplement/'+escape(file)+'" /> </audio>'
                    +'<div class="media-state"></div>'
                    +'<div class="media-controls ui-corner-bottom">'
                    +'<a class="play-pause"><span class="ui-icon"> </span><span class="button-text">play / pause</span></a>'
                    +'<span class="current-time player-display">00:00</span>'
                    ...
                    +'</div>')
                .show();
                $audiocontainer.find('div.media-player').jmeControl();
            } else {
...
var download_pubmed={
    init:function(){
        ...
        $("div.titles").click(function() {
            ...
            $firstcontainer=$this.siblings('table.firstcontainer'),uid=$this.parent('div.items').attr('id').split('-').pop();
        ...
        $("div.flag").click(function() {
            var $this=$(this),uid=$this.parent('div.items').attr('id').substring(4),count=parseInt($('#pubmed-flagged-count').text());
...
var download_pmc={
    init:function(){
        ...
        $("div.titles").click(function() {
            var $this=$(this),$abstract=$this.siblings("div.abstract_container"),$c=$(this).siblings('div.save_container'),
            $firstcontainer=$this.siblings('table.firstcontainer'),uid=$this.parent('div.items').attr('id').split('-').pop(),
            pmcid=$this.parent('div.items').data('pmcid');
            if($firstcontainer.is(':visible')) {
                if($abstract.html()=='') {
                    if($('body').data('click')==true) return false;
                    $('body').data('click',true);
        ...
        $("div.flag").click(function() {
            var $this=$(this),uid=$this.parent('div.items').data('pmcid'),count=parseInt($('#pmc-flagged-count').text());
...
var download_nasa={
    init:function(){
        ...
        $("div.flag").click(function() {
            var $this=$(this),uid=$this.parent('div.items').data('uid'),count=parseInt($('#nasaads-flagged-count').text());
...
var download_arxiv={
    init:function(){
        ...
        $("div.flag").click(function() {
            var $this=$(this),uid=$this.parent('div.items').data('uid'),count=parseInt($('#arxiv-flagged-count').text());
