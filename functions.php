...
function cache_name() {
    ...
    $clipboard = array();
    if(isset($_SESSION['session_clipboard'])) $clipboard = $_SESSION['session_clipboard'];
    if(isset($_SESSION['limit'])) $clipboard[] = $_SESSION['limit'];
    if(isset($_SESSION['orderby'])) $clipboard[] = $_SESSION['orderby'];
    if(isset($_SESSION['display'])) $clipboard[] = $_SESSION['display'];
    ...
    $cache_name = $_SESSION['user_id'].'_file_'.$md5_cache;
    if (empty($isapc)) $cache_name = $temp_dir . DIRECTORY_SEPARATOR . 'lib_' . session_id() . DIRECTORY_SEPARATOR.$cache_name;
    return $cache_name;
}
...
function database_change() {
    ...
    if (isset($tables_arr[0])) $tables = (array) $tables_arr[0];
    if (isset($tables_arr[1])) $tables2 = (array) $tables_arr[1];
...
function cache_start($ch_time) {
    ...
    if (empty($isapc)) {
        if (is_file($cache_name)) $mtime = filemtime($cache_name);
    } else {
        foreach (new APCIterator('user', '/^'.$cache_name.'$/', APC_ITER_MTIME) as $cache_item) {
    ...
    if ($ch_time < $mtime) {
        if (empty($isapc)) {
            ...
            if ($cached_string = apc_fetch($cache_name)) {
                $page = gzuncompress($cached_string);
                echo $page;
                exit();
            }
        }
...
function cache_store() {
    ...
    if (empty($isapc)) {
        ...
        apc_store($cache_name, $bufferContent);
    }
}
...
//        $clean_files = glob($temp_dir . DIRECTORY_SEPARATOR . 'lib_*' . DIRECTORY_SEPARATOR .'*', GLOB_NOSORT);
//        foreach ($clean_files as $clean_file) {
//            if (is_file($clean_file) && is_writable($clean_file))
//                @unlink($clean_file);
//        }
    } else {
        // DELETE USER'S APC CACHE
        foreach (new APCIterator('user', '/.*_desktop_files$/', APC_ITER_KEY, 10000) as $item) {
            apc_delete($item['key']);
        }
        foreach (new APCIterator('user', '/.*_shelf_files$/', APC_ITER_KEY, 10000) as $item) {
            apc_delete($item['key']);
        }
    }
}
...
function save_export_files($files) {
    global $temp_dir;
    $filename = $temp_dir . DIRECTORY_SEPARATOR . 'lib_' . session_id() . DIRECTORY_SEPARATOR . 'exportfiles';
    $export_files = array();
    ...
    if (empty($isapc)) {
        ...
        apc_store($_SESSION['user_id'].'_export_files', $export_files_content);
    }
}
...
function read_export_files($ch_time) {
    ...
    if (!empty($isapc)) {
        if ($export_files = apc_fetch($_SESSION['user_id'].'_export_files'))
            $export_files_array = unserialize(gzuncompress($export_files));
    } else {
...
function show_search_results($result, $select, $display, $shelf_files, $desktop_projects, $tempdbHandle) {
    ...
    if ($display == 'icons') print '<table cellspacing=0 id="icon-container" style="border:0;width:100%">
    ...
    while (list($key, $paper) = each($result)) {
        ...
        if ($display == 'icons') {
            ...
            if (!extension_loaded('gd')) die('<p>&nbsp;Error! Icon view requires GD extension and Ghostscript.</p>');
            
            $first_author = '&nbsp;';
            $auth_arr = explode (';', $paper['authors']);
            $auth_arr2 = explode (',', $auth_arr[0]);
            if (!empty($auth_arr2[0])) $first_author = $auth_arr2[0];
            $etal = '';
            if (count($auth_arr) > 1) $etal = ', et al.';
            ...
            if(!empty($paper['year'])) print ' ('.substr($paper['year'], 0, 4).')';
            print '</div></div>';
            ...
            if (isset($shelf_files) && in_array($paper['id'], $shelf_files)) {
                ...
                print '<div class="update_shelf add">
                    <span class="update_shelf ui-icon ui-icon-circlesmall-close" style="float:left"></span>Shelf&nbsp;</div>';
            }
            ...
            if (isset($_SESSION['session_clipboard']) && in_array($paper['id'], $_SESSION['session_clipboard'])) {
                ...
                print '<div class="update_clipboard add">
                    <span class="update_clipboard ui-icon ui-icon-circlesmall-close" style="float:left"></span>Clipboard&nbsp;</div>';
            }
            ...
            foreach ($desktop_projects as $desktop_project) {
                ...
                if (empty($project_rowid))
                    print '<div class="' . $desktop_project['projectID'] . ' update_project add">
                        <span class="update_project ui-icon ui-icon-circlesmall-close" style="float:left"></span>' . htmlspecialchars($desktop_project['project']) . '&nbsp;</div>';
            ...
            if ($display != 'abstract')
                print PHP_EOL . '<span class="expander ui-icon ui-icon-circlesmall-plus view-' . $display . '" style="margin:0 4px"></span>';
            ...
            if (!empty($paper['authors']))
                print PHP_EOL . '<div class="authors"><span class="author_expander ui-icon ui-icon-circlesmall-plus" style="float:left"></span><div>' . $paper['authors'] . '</div></div>';
    ...
    if ($display == 'icons') print '</td></tr></table>';
}
...
function read_shelf($dbHandle) {
    ...
    if (isset($_SESSION['auth'])) {
        ...
        if (empty($isapc)) {
            ...
            if ($content = apc_fetch($_SESSION['user_id'].'_shelf_files')) {
                ...
                $files_array = $result->fetchAll(PDO::FETCH_COLUMN);
                $result = null;
                apc_store($_SESSION['user_id'].'_shelf_files', gzcompress(serialize($files_array), 1));
            }
        }
        return $files_array;
    }
...
function read_desktop($dbHandle) {
    ...
    if (isset($_SESSION['auth'])) {
        ...
        if (empty($isapc)) {
            ...
            if ($content = apc_fetch($_SESSION['user_id'].'_desktop_files')) {
                $files_array = unserialize(gzuncompress($content));
            } else {
                $id_query = $dbHandle->quote($_SESSION['user_id']);
                $query = $dbHandle->query("SELECT DISTINCT projects.projectID AS projectID,project FROM projects
                ...
                $files_array = $query->fetchAll(PDO::FETCH_ASSOC);
                $query = null;
                apc_store($_SESSION['user_id'].'_desktop_files', gzcompress(serialize($files_array), 1));
            }
        }
        return $files_array;
    }
