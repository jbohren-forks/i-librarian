<?php
#session is disabled for this script due to a bug in Win32, Apache or PHP which is not resolved as of 03/2009
#multiple system() or exec() commands will hang the script if session is activated
#session_start();
...
if (!isset($_GET['commence'])) {
        ...
        if (empty($_GET['directory'])) {
            ...
            function list_dirs($directory) {
                ...
                while (list($key, $value) = each($dir_content)) {
                    if (is_dir($directory . DIRECTORY_SEPARATOR . $value) && is_readable($directory . DIRECTORY_SEPARATOR . $value)
                            && $value != '.' && $value != '..' && $value[0] != '.' && count($directories) < 2001) {
                            ...
                            <table cellspacing="0">
                                <tr>
                                    <td class="select_span" style="line-height: 16px">
                                        ...
                                        <span class="ui-icon ui-icon-radio-<?php print (empty($batchimport_log) || (isset($batchimport_log) && $batchimport_log == '1')) ? 'off' : 'on'  ?>" style="float:left"></span>
                                ...
                                <tr>
                                    <td class="select_span" style="line-height: 16px">
                                        ...
                                        <span class="ui-icon ui-icon-radio-<?php print (isset($batchimport_log) && $batchimport_log == '2') ? 'off' : 'on'  ?>" style="float:left"></span>
    ...
    ########create log file###########
    $log = $temp_dir . DIRECTORY_SEPARATOR . 'i-librarian' . DIRECTORY_SEPARATOR.$_GET['user'].'-librarian-import.log';
                ...
                if (count($doi[0]) < 1) {
                    ...
                    if (!empty($title)) {
                        ...
                        if (isset($_GET['shelf']) && !empty($userID)) {
                            ...
                            if (!empty($isapc)) {
                                apc_delete($userID.'_shelf_files');
                            }
                        }
