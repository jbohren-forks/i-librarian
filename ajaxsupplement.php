...
if (isset($_FILES['form_new_file']) && is_uploaded_file($_FILES['form_new_file']['tmp_name'])) {
    move_uploaded_file($_FILES['form_new_file']['tmp_name'], $temp_dir . DIRECTORY_SEPARATOR . $_POST['filename']);
}
...
if (!empty($_POST['form_new_file_link'])) {
    ...
    file_put_contents($temp_dir . DIRECTORY_SEPARATOR . $_POST['filename'], $pdf_contents);
}

if (!empty($_POST['filename']) && is_writable($temp_dir . DIRECTORY_SEPARATOR . $_POST['filename'])) {

    $uploaded_file_content = file_get_contents($temp_dir . DIRECTORY_SEPARATOR . $_POST['filename'], FILE_BINARY, null, 0, 100);

    if (stripos($uploaded_file_content, '%PDF') === 0) {
        copy($temp_dir . DIRECTORY_SEPARATOR . $_POST['filename'], $library_path . DIRECTORY_SEPARATOR . $_POST['filename']);
...
if (empty($isapc)) {
    ...
    $clean_files = glob($temp_dir . DIRECTORY_SEPARATOR . 'lib_*' . DIRECTORY_SEPARATOR .'?_file_*', GLOB_NOSORT);
    ...
    foreach (new APCIterator('user', '/.*_file_.*/', APC_ITER_KEY, 10000) as $item) {
        apc_delete($item['key']);
    }
}
