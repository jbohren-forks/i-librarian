...
if(!empty($_GET['file'])) {
    ...
    if (is_readable($file_name)) {
        ...
        if ($_SESSION['watermarks'] == 'nocopy') {
            $temp_file = $temp_dir.DIRECTORY_SEPARATOR.'i-librarian'.DIRECTORY_SEPARATOR.$file.'-nocopy.pdf';
            ...
            $temp_file = $temp_dir.DIRECTORY_SEPARATOR.'i-librarian'.DIRECTORY_SEPARATOR.$file.'-confidential.pdf';
            if (!file_exists($temp_file) || filemtime($temp_file) < filemtime($file_name))
        ...
        if (isset($_GET['attachments'])) {
	        ...
	        $integer = sprintf ("%05d", intval($_GET['file']));
                $supfile_arr = glob($path.DIRECTORY_SEPARATOR.'supplement'.DIRECTORY_SEPARATOR.$integer.'*');
            }
