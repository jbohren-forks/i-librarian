...
if (isset($_GET['id'])) {
    ...
    if (empty($_GET['id']) && !empty($_GET['pmcid'])) {
        ...
        foreach ($xml->article->front->{'article-meta'}->{'article-id'} as $articleid) {
            ...
            foreach ($articleid->attributes() as $a => $b) {
                ...
                if ($a == 'pub-id-type' && $b == 'doi') $doi = $articleid[0];
                if ($a == 'pub-id-type' && $b == 'pmid') $pmid = $articleid[0];
            }
        }
        ...
        if (empty($abstract)) $abstract = trim (strip_tags ((string) $xml->article->front->{'article-meta'}->abstract->asXML()));

        foreach ($xml->article->front->{'journal-meta'}->{'journal-id'} as $journalid) {

            foreach ($journalid->attributes() as $a => $b) {

                if ($a == 'journal-id-type' && $b == 'nlm-ta') $journal_abbr = $journalid[0];
            }
        }
        ...
        foreach ($authors->contrib as $author) {
            ...
            foreach ($author->attributes() as $a => $b) {
                ...
                if ($a == 'contrib-type' && $b == 'author') $name_array[] = $author->name->surname.", ".$author->name->{'given-names'};
            }
        }
    ...
    if (!empty($year)) print " (".htmlspecialchars($date).")";
    ...
    if (!empty($affiliation)) print '<div class="authors"><span class="author_expander ui-icon ui-icon-circlesmall-plus" style="float:left"></span><div>'.htmlspecialchars($affiliation).'</div></div>';
    ...
    if (!empty($pmid)) $uid_array[] = 'PMID:'.$pmid;
    ...
    if (!empty($pmid)) $url_array[] = 'http://www.pubmed.org/'.$pmid;
...
<input type="hidden" name="form_new_file_link" value="<?php print !empty($pmcid) ? htmlspecialchars("http://www.ncbi.nlm.nih.gov/pmc/articles/PMC".$pmcid."/pdf") : ""; ?>">
    ...
    if (!empty($doi)) print ' <b>&middot;</b> <a href="'.htmlspecialchars('http://dx.doi.org/'.urlencode($doi)).'" target="_blank">Publishers Website</a>';
