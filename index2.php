...
/////////////// LDAP SETTINGS //////////////////////////////
///This is unmaintained; do not use!!!

$ldap_active = false;    /* activate or not the LDAP login */
$ldap_version = 3;     /* change to match your ldap protocol version */
$ldap_server = "ldap://ldap.mydomain.com"; /* change to ip address of ldap server */
$ldap_port = 389;     /* change to LDAP server port */
$ldap_basedn = "o=something";   /* change to reflect the base distinguished name */
$ldap_rdn = array(
    "users" => "ou=users",
    "groups" => "ou=groups"
);       /* change to reflect the relative dn */
$ldap_cn = "uid=";     /* user common name prefix */
$ldap_admin_cn = "cn=Domain Admins";  /* admin group common name */
$ldap_filter = "memberUid=";   /* authorization filter prefix */
...
if (isset($_GET['action']) && $_GET['action'] == 'signout') {
    ...
    if (!empty($isapc)) {
        ...
        foreach (new APCIterator('user', '/^'.$_SESSION['user_id'].'_.*/', APC_ITER_KEY, 1000) as $cache_item) {
            apc_delete($cache_item['key']);
        }
    }
    $_SESSION = array();
    session_destroy();
...
if (!$ldap_active && isset($_POST['form']) && $_POST['form'] == 'signup' && !empty($_POST['user']) && !empty($_POST['pass']) && !empty($_POST['pass2'])) {
    ...
    if ($_POST['pass'] == $_POST['pass2']) {
        ...
        if (isset($_SESSION['auth'])) {
            ...
            @mkdir($temp_dir . DIRECTORY_SEPARATOR . 'i-librarian');
        }
...
if (isset($_POST['form']) && $_POST['form'] == 'signin' && !empty($_POST['user']) && !empty($_POST['pass']) && !isset($_SESSION['auth'])) {
    ...
    if ($ldap_active) {
        ...
        if ($ldap_bind = @ldap_bind($ldap_connect, $ldap_dn, $password)) {
            ...
            $count = $dbHandle->query("SELECT count(*) FROM shelves WHERE user=$username_quoted LIMIT 1");
            $rows = $count->fetchColumn();
            $count = null;
            ...
            $local_uid = $dbHandle->query("SELECT id FROM shelves WHERE user=$username_quoted LIMIT 1");
            $user = $local_uid->fetch(PDO::FETCH_ASSOC);
...
<?php include_once 'style.php'; ?>
        ...
        <script type="text/javascript" src="js/jme/mm.full.min.js?v=<?php print $version ?>"></script>
        <script type="application/swf" src="js/jme/player.swf?v=<?php print $version ?>"></script>
        <script type="text/javascript" src="js/jqueryFileTree.js?v=<?php print $version ?>"></script>
        <script type="text/javascript" src="js/yui/yahoo-dom-event.js?v=<?php print $version ?>"></script>
        <script type="text/javascript" src="js/yui/element-min.js?v=<?php print $version ?>"></script>
        <script type="text/javascript" src="js/yui/uploader-min.js?v=<?php print $version ?>"></script>
...
if (isset($_SESSION['auth'])) {
    ...
    ?>
                ...
                <div id="credits" style="position: absolute;right:10px;bottom:10px;cursor:pointer">
                    I, Librarian <?php print $version ?> &copy; 2012 Martin Kucej &middot; GPLv3
                </div>
            </div>
